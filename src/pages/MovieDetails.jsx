    ...
    .filter((review) => {
      ...
      return (
        ...
        !cleanedContent.includes(
          "This reviewer didn't provide a detailed comment"
        )
      );
    })
    .slice(0, 3);
      ...
      <div className="relative h-[70vh] min-h-[500px] overflow-hidden">
        ...
        <div className="relative h-full flex flex-col justify-end pb-12 px-4 md:px-10 lg:px-20 max-w-7xl mx-auto">
          ...
          <div className="max-w-2xl">
            ...
            {movie.tagline && (
              <p className="text-[#c9a092] text-lg italic mb-6">
                "{movie.tagline}"
              </p>
            )}
          </div>
                ...
                <div className="space-y-2">
                  {[5, 4, 3, 2, 1].map((stars) => {
                    ...
                    if (stars === 5) {
                      percentage = Math.max(
                        10,
                        Math.min(60, (avgRating - 3) * 20 + 30)
                      );
                    } else if (stars === 4) {
                      percentage = Math.max(
                        15,
                        Math.min(40, (avgRating - 2.5) * 15 + 25)
                      );
                    } else if (stars === 3) {
                      percentage = Math.max(
                        10,
                        Math.min(35, (avgRating - 2) * 10 + 20)
                      );
                    } else if (stars === 2) {
                      percentage = Math.max(
                        5,
                        Math.min(25, (avgRating - 1.5) * 8 + 15)
                      );
                    } else {
                      percentage = Math.max(
                        2,
                        Math.min(20, (avgRating - 1) * 5 + 10)
                      );
                    }
              ...
              {filteredReviews.length > 0 ? (
                <div className="space-y-4">
                  {filteredReviews.map((review) => (
                    <div
                      key={review.id}
                      className="bg-[#2a1a15] rounded-lg p-4"
                    >
